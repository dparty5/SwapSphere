...
namespace project_SwapSphere_server.Service
{
    internal class ChatService
    {
        ...
        public CancellationToken CreateConnection(Client user, Swap swap, WebSocket webSocket)
        {
            var room = ChatRooms.FirstOrDefault(x => x.Swap.Id == swap.Id);
            if (room is null)
                ChatRooms.Add(room = new(swap));
